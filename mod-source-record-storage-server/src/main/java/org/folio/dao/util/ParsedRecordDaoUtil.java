...
public final class ParsedRecordDaoUtil {
  ...
  /**
   * Searches for {@link ParsedRecord} by id using {@link CommonQueryExecutor}
   *
   * @param queryExecutor query executor
   * @param id            id
   * @param recordType    record type to find
   * @return future with optional ParsedRecord
   */
  public static Future<Optional<ParsedRecord>> findById(CommonQueryExecutor queryExecutor, String id,
  ...
  /**
   * Saves {@link ParsedRecord} to the db table defined by {@link RecordType} using {@link CommonQueryExecutor}
   *
   * @param queryExecutor query executor
   * @param parsedRecord  parsed record
   * @param recordType    record type to save
   * @return future with updated ParsedRecord
   */
  public static Future<ParsedRecord> save(CommonQueryExecutor queryExecutor, ParsedRecord parsedRecord, RecordType recordType) {
    UUID id = UUID.fromString(parsedRecord.getId());
    JsonObject content = normalize(parsedRecord.getContent());
   ...
   * @return future of updated ParsedRecord
   */
  public static Future<ParsedRecord> update(CommonQueryExecutor queryExecutor,
                                            ParsedRecord parsedRecord, RecordType recordType) {
    UUID id = UUID.fromString(parsedRecord.getId());
    JsonObject content = normalize(parsedRecord.getContent());
