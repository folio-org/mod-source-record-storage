...
import org.folio.rest.jaxrs.resource.LbSourceStorageBatch;
...
public class LbSourceStorageBatchImpl implements LbSourceStorageBatch {
  private static final Logger LOG = LoggerFactory.getLogger(LbSourceStorageBatchImpl.class);
  ...
  public LbSourceStorageBatchImpl(Vertx vertx, String tenantId) { //NOSONAR
    SpringContextUtil.autowireDependencies(this, Vertx.currentContext());
    this.tenantId = TenantTool.calculateTenantId(tenantId);
  }

  @Override
  public void postLbSourceStorageBatchRecords(RecordCollection entity, Map<String, String> okapiHeaders,
        ...
        recordService.saveRecords(entity, tenantId)
          .map(recordsBatchResponse -> {
            if (!recordsBatchResponse.getRecords().isEmpty()) {
              return PostLbSourceStorageBatchRecordsResponse.respond201WithApplicationJson(recordsBatchResponse);
            } else {
              LOG.error("Batch of records was processed, but records were not saved, error messages: {}", recordsBatchResponse.getErrorMessages());
              return PostLbSourceStorageBatchRecordsResponse.respond500WithApplicationJson(recordsBatchResponse);
            }
          })
  ...
  @Override
  public void putLbSourceStorageBatchParsedRecords(RecordCollection entity, Map<String, String> okapiHeaders,
                                                 Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) {
    vertxContext.runOnContext(v -> {
      try {
        populateRecordsMetaData(entity.getRecords(), okapiHeaders);
        recordService.updateParsedRecords(entity, tenantId)
          .map(parsedRecordsBatchResponse -> {
            if (!parsedRecordsBatchResponse.getParsedRecords().isEmpty()) {
              return PutLbSourceStorageBatchParsedRecordsResponse.respond200WithApplicationJson(parsedRecordsBatchResponse);
            } else {
              ...
              return PutLbSourceStorageBatchParsedRecordsResponse.respond500WithApplicationJson(parsedRecordsBatchResponse);
            }
          })
          .map(Response.class::cast)
          .otherwise(ExceptionHelper::mapExceptionToResponse)
