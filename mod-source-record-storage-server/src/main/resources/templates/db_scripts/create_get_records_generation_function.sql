...
CREATE OR REPLACE FUNCTION get_highest_generation(matchedId uuid, jobSnapshotId uuid)
...
SELECT MAX(records.jsonb ->> 'generation') into generation
	FROM records
	INNER JOIN snapshots ON records.snapshotId = snapshots.id
    ...
    AND (snapshots.jsonb -> 'metadata' ->> 'updatedDate')::timestamp with time zone <
      (SELECT snapshots.jsonb ->> 'processingStartedDate'
	      FROM snapshots
	      WHERE (snapshots.jsonb ->> 'jobExecutionId')::uuid = jobSnapshotId)::timestamp with time zone;
RETURN generation;
END;
