...
SELECT records._id,
       json_build_object('recordId', records.jsonb ->> 'id',
         ...
         AS jsonb
FROM records
       JOIN raw_records ON records.rawrecordid = raw_records._id
       LEFT JOIN marc_records ON records.parsedrecordid = marc_records._id
WHERE records.parsedrecordid IS NOT NULL;
