...
public class LbRecordApiTest extends AbstractRestVerticleTest {
  ...
  public void suppressFromDiscoveryByInstanceIdSuccess(TestContext context) {
    ...
    RestAssured.given()
      .spec(spec)
      .when()
      .put(SOURCE_STORAGE_RECORDS_PATH + "/" + instanceId + "/suppressFromDiscovery?idType=INSTANCE&suppress=true")
      .then().log().all()
      .statusCode(HttpStatus.SC_OK);
    async.complete();
  }
  ...
  public void suppressFromDiscoveryByInstanceIdNotFound(TestContext context) {
    ...
    RestAssured.given()
      .spec(spec)
      .when()
      .put(SOURCE_STORAGE_RECORDS_PATH + "/" + UUID.randomUUID().toString() + "/suppressFromDiscovery?idType=INSTANCE&suppress=true")
      .then().log().all()
      .statusCode(HttpStatus.SC_NOT_FOUND);
    async.complete();
  }
