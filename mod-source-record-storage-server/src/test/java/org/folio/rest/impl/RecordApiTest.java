...
public class RecordApiTest extends AbstractRestVerticleTest {
  ...
  public void shouldReturnRecordsOnGetBySpecifiedSnapshotId(TestContext testContext) {
    ...
    RestAssured.given()
      .spec(spec)
      .when()
      .get(SOURCE_STORAGE_RECORDS_PATH + "?query=(state=ACTUAL and snapshotId=" + record_2.getSnapshotId() + ")")
      .then()
      .statusCode(HttpStatus.SC_OK)
  ...
  public void shouldReturnLimitedCollectionWithActualStateOnGetWithLimit(TestContext testContext) {
    ...
    RestAssured.given()
      .spec(spec)
      .when()
      .get(SOURCE_STORAGE_RECORDS_PATH + "?query=state=ACTUAL&limit=2")
      .then()
      .statusCode(HttpStatus.SC_OK)
  ...
  public void shouldReturnSortedSourceRecordsOnGetWhenSortByOrderIsSpecified(TestContext testContext) {
    ...
    List<SourceRecord> sourceRecordList = RestAssured.given()
      .spec(spec)
      .when()
      .get(SOURCE_STORAGE_SOURCE_RECORDS_PATH + "?query=snapshotId=" + snapshot_2.getJobExecutionId() + " sortBy order/number")
      .then().log().all()
      .statusCode(HttpStatus.SC_OK)
  ...
  public void shouldReturnParsedResultsOnGetWhenNoQueryIsSpecifiedWithActualState(TestContext testContext) {
    ...
    RestAssured.given()
      .spec(spec)
      .when()
      .get(SOURCE_STORAGE_SOURCE_RECORDS_PATH + "?query=state=ACTUAL")
      .then()
      .statusCode(HttpStatus.SC_OK)
      .body("totalRecords", is(1))
      .body("sourceRecords*.parsedRecord", notNullValue())
  ...
  public void shouldReturnSortedRecordsOnGetWhenSortByOrderIsSpecified(TestContext testContext) {
    ...
    List<Record> records = RestAssured.given()
      .spec(spec)
      .when()
      .get(SOURCE_STORAGE_RECORDS_PATH + "?query=snapshotId=" + snapshot_2.getJobExecutionId() + " sortBy order/number")
      .then().log().all()
      .statusCode(HttpStatus.SC_OK)
